...
/***********************************************************
-  Description  :  通过递归获取目录内的音乐并插入到链表
-  Calls  :  opendir()、calloc()、readdir()、
			 closedir()、snprintf()、mus_inlist.c->inlist()
-  Iuput arg :  1、储存相册音乐的头指针 2、目录名的地址
-  Return  :  返回新的头指针
-  Others  :  none
*************************************************************/
MUS get_mus(MUS head,char **argv)
{
	...
	while(1)
	{
		...
		if(i > 1)
		{
			...
			{
				/* 插入链表 */
				head = mus_inlist(head,dir->d_name,*argv);
			}
		}
		i++;
	}
