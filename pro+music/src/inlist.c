...
/***********************************************************
-  Description  :  采用尾插法把音乐名插入链表
-  Calls  :  snprintf()、malloc()、strcpy()
-  Iuput arg :  1、储存音乐链表的头指针 2、音乐名 3、当前路径名
-  Return  :  返回新的头指针
-  Others  :  none
*************************************************************/
MUS mus_inlist(MUS head,char *d_name,char *argv)
{
	char name[40] = {};
	/* 把当前路径和文件名拼成真正音乐路径 */
	snprintf(name,40,"%s%s",argv,d_name);
	
	/* 创建一个新节点并把音乐名拷贝给新节点内的name */
	MUS new_node = malloc(sizeof(mus));
	strcpy(new_node->pname,name);
